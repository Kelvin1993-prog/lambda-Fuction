...
# Your code goes here
weak_passwords = list(filter(lambda x: len(x[0:]) < 6, passwords))
medium_passwords = list(filter(lambda x: len(x[0:]) >=6 and len(x[0:]) <=10, passwords))
strong_passwords = list(filter(lambda x: len(x[0:]) > 10, passwords))